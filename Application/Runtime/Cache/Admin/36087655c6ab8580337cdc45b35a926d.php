...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			...
			<!-- <a style="text-decoration:none" class="btn btn-primary radius" title="添加资讯"  onclick="news_add('添加资讯','<?php echo U("News/newsAdd");?>')" href="javascript:;"> -->
			<a class="btn btn-primary radius" onclick="news_add('添加资讯','<?php echo U("News/newsAdd");?>')" href="javascript:;" title="添加资讯">
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			...
			<?php if(is_array($news_list)): foreach($news_list as $key=>$news): ?><tr class="text-c">
					...
					<td class="f-14 td-manage">
						...
						<a style="text-decoration:none" class="ml-5" onClick="news_edit('资讯编辑','<?php echo U("News/newsEdit");?>?id=<?php echo ($news[news_id]); ?>')" href="javascript:;" title="编辑">
							<i class="Hui-iconfont">&#xe6df;</i>
						</a>
...
function news_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		var url = "<?php echo U('News/delete');?>";
		$.ajax({
			type: 'POST',
			url: url,
			data:{
